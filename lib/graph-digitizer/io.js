...
export async function exportProjectExcel(project, options = {}) {
  ...
  if (eligible.length >= 2) {
    ...
    const summarySheet = [
      ...
      [],
      ["Curve", "RMSE", "MAE", "Max Abs Error", "Area Error", "Similarity %", "Points Compared", "Points Skipped"],
      ...results.map(({ curve, summary }) => [
        curve.name, summary.rmse, summary.mae, summary.maxAbsError, summary.areaError, summary.similarity, summary.compared, summary.skipped,
      ]),
    ];
    XLSX.utils.book_append_sheet(workbook, XLSX.utils.aoa_to_sheet(summarySheet), "Comparison Summary");

    const pointRows = [
      ["Curve", xHead, `${yHead} (reference)`, `${yHead} (interpolated)`, "Error", "Absolute Error", "Status"],
      ...results.flatMap(({ curve, rows }) => rows.map((row) => [curve.name, row.x, row.yRef, row.yInterp, row.error, row.absError, row.status])),
    ];
