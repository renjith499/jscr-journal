...
export const METHODS = {
  rom: {
    name: "Voigt–Reuss / Rule of Mixtures",
    short:
      "Direct Rule of Mixtures (Voigt) axially and inverse Rule of Mixtures (Reuss) transversely",
  },
  hill: {
...
export function calculateComposite(input, requestedMethod) {
  ...
  const strengthMix = (key, label, direction) => {
    const missing = [
      [`fiber ${label}`, fiber[key]],
      [`matrix ${label}`, matrix[key]],
    ]
    ...
    if (method === "moriTanaka")
      return result(
        null,
        "Linear-elastic Mori–Tanaka does not predict yield or failure strength",
        ["a nonlinear matrix law and failure criterion"],
      );
    if (missing.length)
      return result(null, `${METHODS[method].name}: ${label}`, missing);
    const a = fiber[key];
    ...
    if (method === "hill")
      return result(
        (voigt(a, b, vf, vm) + reuss(a, b, vf, vm)) / 2,
        `${label}_H = (${label}_V + ${label}_R)/2 (screening estimate)`,
      );
    if (method === "halpinTsai" && direction !== "axial") {
      const xi = Math.max(
        0.000001,
        valid(input.halpinXiTransverse) ? n(input.halpinXiTransverse) : 2,
      );
      return result(
        halpinTsai(a, b, vf, xi),
        `${label} Halpin–Tsai screening estimate, ξ = ${xi}`,
      );
    }
    if (method === "chamis" && direction !== "axial")
      return result(
        chamis(a, b, vf),
        `${label} Chamis-form screening estimate`,
      );
    return result(
      direction === "axial" ? voigt(a, b, vf, vm) : reuss(a, b, vf, vm),
      direction === "axial"
        ? `${label} direct Rule of Mixtures`
        : `${label} inverse Rule of Mixtures`,
    );
  };
  const density = two("density", "density", "ρ_c = V_fρ_f + V_mρ_m", (a, b) =>
    voigt(a, b, vf, vm),
  );
  const Xt = strengthMix("tensile1", "direction-1 tensile yield/strength", "axial");
  const Xc = strengthMix("compression1", "direction-1 compressive yield/strength", "axial");
  const Yt = strengthMix("tensile2", "direction-2 tensile yield/strength", "transverse");
  const Yc = strengthMix("compression2", "direction-2 compressive yield/strength", "transverse");
  const Zt = strengthMix("tensile3", "direction-3 tensile yield/strength", "transverse");
  const Zc = strengthMix("compression3", "direction-3 compressive yield/strength", "transverse");
