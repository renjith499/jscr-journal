...
import { submitCDPUsageEmail } from "@/lib/cdp-studio/feedback";

export function CDPEmailGateModal({ onSuccess, onClose }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      await submitCDPUsageEmail(email, usage);
      onSuccess(email);
    } catch {
      ...
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-soft dark:bg-slate-900">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-300">
              Your material inputs and CDP tables stay in your browser — they are never uploaded. We ask for an email once per visit so we understand who is using this tool and how.
            </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <label>
            ...
            <input value={usage} onChange={(event) => setUsage(event.target.value)} placeholder="e.g. concrete beam using Abaqus 2020" className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-accent focus:ring-4 focus:ring-cyan-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
          </label>
